...
CURRENT_CAPACITY=$(cat info.txt | grep CurrentCapacity | awk '{printf ("%f", $3)}')
MAX_CAPACITY=$(cat info.txt | grep MaxCapacity | awk '{printf ("%f", $3)}')
DESIGN_CAPACITY=$(cat info.txt | grep DesignCapacity | awk '{printf ("%f", $3)}')
TEMPERATURE=$(cat info.txt | grep Temperature | awk '{printf ("%.1f", $3/100.00)}')
MANUFACTURE_DATE=$(cat info.txt | grep ManufactureDate | awk '{printf ("%f", $3)}')
...
HEALTH=$(echo $MAX_CAPACITY $DESIGN_CAPACITY | awk '{printf ("%.1f", $1/$2 * 100)}')
CHARGE=$(echo $CURRENT_CAPACITY $MAX_CAPACITY | awk '{printf ("%.1f", $1/$2 * 100)}')
CELLS=$(python -c "f='●'*($CHARGE/10) + '○'*(10-$CHARGE/10);print f")

CYCLE_COUNT=$(cat info.txt | grep -e '"CycleCount" =' | awk '{printf ("%d", $3)}')
